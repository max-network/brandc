...
```
feature branch ──PR──> develop ──(auto-pr.yml)──> standing PR ──merge──> main ──> npm
```

Target **`develop`**, not `main`. Every push to `develop` refreshes a standing "promote
develop → main" PR, so `main` stays a release branch rather than a working one. Same flow as
`worker-utils` and `mcp-http`.

**Releasing is bumping the version in your PR.** A merge to `main` publishes whatever
`package.json` says; if that version is already on npm the run is a green no-op. Nothing
infers the bump for you, so an additive change to the contract can take the minor it deserves.

Adding a name to `CONTRACT_*` is an additive contract change: every brand typed
`satisfies ContractBrand` must supply it, and the pinned counts in `test/contract.test.mjs` and
the token count above must move in the same PR. That friction is deliberate.
